...
public class BTree {
	...
	public void inserir(int key, Object object) {
		// noh raiz
		NOB rootNode = raiz; 
		//verifica se o noh existe na árvore
		if (!atualizar(raiz, key, object)) {
			//insercao com split
			if (rootNode.mNumChaves == (2 * D - 1)) {
				NOB newRootNode = new NOB();
				raiz = newRootNode;
				newRootNode.mEhNoFolha = false;
				raiz.mNosFilhos[0] = rootNode;
				splitChildNode(newRootNode, 0, rootNode); // Split o noh pai, movendo a chave mediana para cima no novo noh criado.
	...
	//verifica se a chave existe na arvore
	private boolean atualizar(NOB node, int key, Object object) {
		while (node != null) {
			int i = 0;
			while (i < node.mNumChaves && key > node.mChaves[i]) {
				i++;
			}
			//a chave existe
			if (i < node.mNumChaves && key == node.mChaves[i]) {
				node.mObjects[i] = object;
				return true;
			}
			//chegou numa folha, pode inserir.
			if (node.mEhNoFolha) {
				return false;
			} //continua a busca, muda de página. 
			else {
				node = node.mNosFilhos[i];
			}
		}
		return false;
	}
...
////This method will only be called if node is full; node is the i-th child of parentNode.
	public void splitChildNode(NOB parentNode, int i, NOB node) {
		NOB newNode = new NOB();
		newNode.mEhNoFolha = node.mEhNoFolha;
		newNode.mNumChaves = D - 1;
		for (int j = 0; j < D - 1; j++) { // Copy the last T-1 elements of node
											// into newNode.
			newNode.mChaves[j] = node.mChaves[j + D];
			newNode.mObjects[j] = node.mObjects[j + D];
		}
		if (!newNode.mEhNoFolha) {
			for (int j = 0; j < D; j++) { // Copy the last T pointers of node
											// into newNode.
				newNode.mNosFilhos[j] = node.mNosFilhos[j + D];
			}
			for (int j = D; j <= node.mNumChaves; j++) {
				node.mNosFilhos[j] = null;
			}
		}
		for (int j = D; j < node.mNumChaves; j++) {
			node.mChaves[j] = 0;
		...
		for (int j = parentNode.mNumChaves; j >= i + 1; j--) {
			parentNode.mNosFilhos[j + 1] = parentNode.mNosFilhos[j];
		}
		parentNode.mNosFilhos[i + 1] = newNode;
		for (int j = parentNode.mNumChaves - 1; j >= i; j--) {
			parentNode.mChaves[j + 1] = parentNode.mChaves[j];
			parentNode.mObjects[j + 1] = parentNode.mObjects[j];
		}
		parentNode.mChaves[i] = node.mChaves[D - 1];
		parentNode.mObjects[i] = node.mObjects[D - 1];
		node.mChaves[D - 1] = 0;
		node.mObjects[D - 1] = null;
		parentNode.mNumChaves++;
	}
	
////Insere uma chava na B-Tree. (Serah automaticamente inserido num noh folha). 
void insertIntoNonFullNode(NOB node, int key, Object object) {
	int i = node.mNumChaves - 1;
	if (node.mEhNoFolha) {
		// Since node is not a full node insert the new element into its proper place within node.
		while (i >= 0 && key < node.mChaves[i]) {
			node.mChaves[i + 1] = node.mChaves[i];
		...
		i++;
		if (node.mNosFilhos[i].mNumChaves == (2 * D - 1)) {
			splitChildNode(node, i, node.mNosFilhos[i]);
			if (key > node.mChaves[i]) {
				i++;
			}
		}
		insertIntoNonFullNode(node.mNosFilhos[i], key, object);
	}
}
...
public void print(){ 
	...
	String printBTree(NOB node) {
		String string = "";
		if (node != null) {
			if (node.mEhNoFolha) {
				for (int i = 0; i < node.mNumChaves; i++) {
					string += node.mObjects[i] + ", ";
				}
			} else {
				int i;
				for (i = 0; i < node.mNumChaves; i++) {
					string += printBTree(node.mNosFilhos[i]);
					string += node.mObjects[i] + ", ";
				}
				string += printBTree(node.mNosFilhos[i]);
			}
